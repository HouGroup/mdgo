...
def trajectory(nvt_run, li_atom, run_start, run_end, species, selection_dict,
    ...
    for ts in trj_analysis:
        selection = (
                "(" + selection_dict.get(species) + ") and (around "
                + str(distance) + " index "
                + str(li_atom.id - 1) + ")"
        )
    ...
    for ts in trj_analysis:
        ...
        if write and cn_values["total"][time_count] == structure_code:
            ...
            if a > 1 - write_freq:
                ...
                path = (
                        write_path + str(li_atom.id) + "_"
                        + str(int(ts.time)) + "_"
                        + str(structure_code) + ".xyz"
                )
...
def num_of_neighbor_one_li_simple(
    ...
    for ts in trj_analysis:
        selection = (
                "(" + select_dict.get(species) + ") and (around "
...
def num_of_neighbor_one_li_simple_extra(nvt_run, li_atom, species, select_dict,
    ...
    for ts in trj_analysis:
        selection = (
                "(" + select_dict.get(species) + ") and (around "
                + str(distance) + " index " + str(li_atom.id - 1) + ")"
        )
        shell = nvt_run.select_atoms(selection, periodic=True)
        shell_len = len(shell)
        if shell_len == 0:
            ...
            if shell_species_len == 0:
                ...
                ec_select = (
                        "(" + select_dict.get("EC") + ") and (around "
                        + str(3) + " index " + str(li_atom.id - 1) + ")"
                )
                emc_select = (
                        "(" + select_dict.get("EMC") + ") and (around "
                        + str(3) + " index " + str(li_atom.id - 1) + ")"
                )
...
def num_of_neighbor_one_li_simple_extra_two(nvt_run, li_atom, species_list,
    ...
    for ts in trj_analysis:
        ...
        selection = (
                "(" + select_dict.get("anion") + ") and (around "
...
def write_out(li_pos, selection, element_id_dict, path):
    ...
    for atom in selection:
        ...
        for i in range(3):
            ...
            locs.append(loc)
        line = (
                element_id_dict.get(int(atom.type)) + ' '
