...
class MdRunTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        bn_fec_dir = pathlib.Path("test_files/BN_FEC_LiPF6")
        cls.data_file = bn_fec_dir / "BN_FEC_LiPF6_split_cation.data"
        cls.unwrapped_dcd = bn_fec_dir / "data_unwrapped_nvt.dcd"
        select_dict = {"cation": "type 22", "anion": "type 20"}
        cls.md_run = MdRun(cls.data_file, cls.unwrapped_dcd,
                           500, 10, "BN_FEC_LiPF6", select_dict)
        cls.res_mass_dict = {'Li': 6.94, 'PF6': 144.97,
                             'FEC': 106.05, 'BN': 69.105}
        cls.md_autorun = \
            MdRun.auto_constructor(cls.data_file, cls.unwrapped_dcd,
                                   500, 10, "BN_FEC_LiPF6", cls.res_mass_dict,
                                   "Li", "PF6", "P", ["BN", "FEC"])
    ...
    def test_auto_constructor(self):
        ...
        np.testing.assert_array_almost_equal(run.cond_array, autorun.cond_array)

    """
    This test should be uncommented when the ion_pairing and shell_functions are merged.
    """
